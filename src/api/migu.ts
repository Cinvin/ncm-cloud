...
async function search(keyword: string, searchType: number) {
	...
	const url =
		...
		return await ipcRenderer.invoke('get-data',url)
		.then((response: any) => {
...
export function searchSinger(keyword: string) {
	return search(keyword, 1).then((res: any) => {
		...
		// ]
		console.log(res)
		if (res && res.singerResultData.result) {
			return res.singerResultData.result
		}
		return []
	})
		.catch(() => { return [] })
...
export function searchAlbum(keyword: string) {
	return search(keyword, 2).then((res: any) => {
		...
		// 	},
		// ]
		if (res && res.albumResultData.result) {
			return res.albumResultData.result
		}
		return []
	})
		.catch(() => { return [] })
...
async function getSingerSongs(id: string) {
	...
	while (true) {
		let response = await ipcRenderer.invoke('get-data',url)
		// console.log(response.data.contentItemList[0].itemList)
		let itemlist = response.data.contentItemList[0].itemList
...
async function getSingerAlbums(id: string) {
	...
	while (true) {
		...
		let itemlist = response.data.contentItemList[1].itemList
...
export async function getAlbumSongs(id: string, resourceType: string) {
	...
	let response = await ipcRenderer.invoke('get-data',url)
	if (response.resource[0] && response.resource[0].songItems) {
		for (let item of response.resource[0].songItems) {
